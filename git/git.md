...
`.gitignore` 文件用来告诉 Git 哪些文件或文件夹**不需要**被纳入版本控制。本文档系统性地总结了所有匹配规则，并附有大量实例。
...
| 空行 | 被忽略（可作为分隔符） | ` ` |
| `#` 开头 | 注释行 | `# 这是注释` |
| 普通文件名 | 精确匹配该文件 | `test.log` |
| 文件夹名 + `/` | 匹配整个文件夹及其所有内容 | `build/` |
| `*` | 匹配 0 个或多个任意字符（不包括 `/`） | `*.tmp` |
| `?` | 匹配 1 个任意字符（不包括 `/`） | `?.txt` |
| `**` | 匹配任意层级的目录（包括 0 层） | `**/logs/` |
| `[]` | 匹配括号内的任一字符（类似正则字符集） | `[abc].txt` |
...
| 写法 | 含义 |
|------|------|
| `logs` | 忽略**任意目录**下名为 `logs` 的文件或文件夹 |
| `logs/` | 忽略**任意目录**的 `logs` 文件夹（及其所有子内容） |
| `/logs/` | 忽略**根目录**的 `logs` 文件夹 |
| `/logs/*` | 忽略**根目录**的 `logs` 文件夹中的所有文件 |
| `src/logs/` | 只忽略 `src/` 下的 `logs/` 文件夹 |
| `**/logs/` | 忽略**任何层级**下的 `logs` 文件夹（如 `a/logs/`、`a/b/logs/`） |

注意：

1. 用/开头表示从仓库根目录开始匹配，忽略/开头则表示匹配任意层级的目录。
2. 使用 `**` 可以匹配任意层级的目录，但通常不会和 `/` 一起使用，例如不会有`/**/logs/`。
3. 以 `/` 结尾表示匹配的是目录，而不是文件。没有 `/` 结尾则匹配文件和目录。

---

## 3. 否定规则（重新包含）

使用 `!` 开头表示**不忽略**（即强制跟踪已被忽略规则匹配的文件）。

```gitignore
# 示例：忽略所有 .log 文件，但保留 important.log
*.log
!important.log
```
