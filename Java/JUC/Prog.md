...
`public V get()`：同步等待 futuretask 执行完毕的结果，如果在线程中获取另一个线程执行结果，会阻塞等待，用于线程同步
...
Monitor 被翻译为监视器或管程，它是操作系统提供的，真正实现锁功能的类。
...
* 写屏障会确保指令重排序时，不会将写屏障之前的代码排在写屏障之后（即不会将num=2移到read-true之后）
...
1. 分配空间
2. 对分配的空间初始化
3. 将INSTANCE指向初始化好的空间

其可以被指令重排序为：

1. 分配空间
2. 将INSTANCE指向初始化好的空间
3. 对分配的空间初始化

![alt text](../pic/DCL-1.png)

当线程进行第一次检查的时候，代码读取到instance不为null时，instance引用的对象有可能还没有完成初始化。
...
引入 volatile，来避免指令重排，从而保证单例模式的线程安全性：
...
* CAS 体现的是**无锁并发、无阻塞并发**，线程CAS失败也不会陷入阻塞，线程不需要频繁切换状态（上下文切换，系统调用）
* CAS 是基于乐观锁的思想

#### CAS 缺点：

- CAS失败后不阻塞，所以往往进行循环尝试，如果比较不成功一直在循环，最差的情况某个线程一直取到的值和预期值都不一样，就会无限循环导致饥饿。由于是循环操作，占用cpu，所以**使用 CAS 线程数不要超过 CPU 的核心数**
- 只能保证一个共享变量的原子操作
...
* synchronized 是从悲观的角度出发：总是假设最坏的情况，每次去拿数据的时候都认为别人会修改，所以每次在拿数据的时候都会上锁，这样别人想拿这个数据就会阻塞（共享资源每次只给一个线程使用，其它线程阻塞，用完后再把资源转让给其它线程），因此 synchronized 也称之为悲观锁，ReentrantLock 也是一种悲观锁，性能较差，适合多线程同时写的操作多的场景。
* CAS 是从乐观的角度出发：总是假设最好的情况，每次去拿数据的时候都认为别人不会修改，所以不会上锁，但是在更新的时候会判断一下在此期间别人有没有去更新这个数据。**如果别人修改过，则获取现在最新的值，如果别人没修改过，直接修改共享数据的值**，CAS 这种机制也称之为乐观锁，综合性能较好，适合多线程同时写的操作少的场景。
...
优化核心思想：LongAdder的基本思路就是分散热点，将value值分散到一个数组中，不同线程会命中到数组的不同槽中，各个线程只对自己槽中的那个值进行CAS操作，这样热点就被分散了，冲突的概率就小很多。 如果要获取真正的long值，只要将各个槽中的变量值累加返回即可。
...
一个很好的讲解：https://www.cnblogs.com/waterystone/p/4920797.html
...
相较于下文，一个更好的教程：https://blog.csdn.net/qq_43509535/article/details/122913153
