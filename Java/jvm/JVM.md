...
这里先补充一下，jvm虚拟机在执行字节码指令时，会用到栈帧的两个结构：
...
* 该实现继承ClassLoader抽象类，'父类'为引导类加载器
...
* 两种**声明方式**
   ...
   String s2 = new String("string 1"); //new方式
   String s3 = new String("string 1");

   System.out.println(s1 == s2);   //false
   System.out.println(s2 == s3);   //false
   ``` 
   如果是字面量方式，实际的存储类型在字符串常量池中，两个相同的字面量引用指向同一个。而两个引用使用new的方式会创建两个类在堆中。
...
* 总的来说，String可能存在的位置有两个：字符串常量池或者堆中独立的一个类。
