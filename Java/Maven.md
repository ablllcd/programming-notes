...
maven提供了一套标准的项目结构和构建流程，开发者只需要按照maven的规范编写代码，maven就能完成项目的构建。
...
- pom.xml：项目配置文件，包含项目信息，依赖信息，构建信息等
- src/main/java：存放项目源代码
- src/main/resources：存放项目资源文件，如配置文件，图片等
- src/test/java：存放测试代码
- src/test/resources：存放测试资源文件
- target：项目构建输出目录，存放编译后的类文件和打包

上述结构是单模块项目结构，maven也支持多模块项目结构：
...
Maven 的构建流程是基于 **生命周期（Lifecycle）** 的，它定义了一系列标准阶段，每个阶段对应一组任务（Goals）。构建流程可以概括为：
1. clean：清理上一次构建的输出
2. build：编译、测试、打包、安装、部署。
...
Maven 支持将大型项目拆分为多个模块，每个模块可以独立开发、测试和构建。模块化管理的好处包括：
- 提高代码复用性：模块可以被多个项目共享
- 简化依赖管理：每个模块可以有自己的依赖，避免冲突
- 加快构建速度：只需构建修改过的模块

模块化主要包括两个核心概念： **继承机制** 和 **聚合机制**。

继承机制允许子模块继承父模块的配置，例如依赖版本、插件配置等，从而实现统一管理。

聚合机制允许将多个模块组合在一起进行构建，方便整体管理和发布。
...
plugin是执行具体构建任务的工具，例如编译代码，打包项目,它们是可执行的任务。而maven的生命周期就是由一系列plugin组成的。

dependency是编写代码时需要引用别人的函数，而plugin则是构建项目时的一些辅助工具，不会影响代码。
...
Packing字段指明项目打包的方式，也是声明该模块的`最终产物`。

常见的打包方式有：
1. **jar**：Java Archive，打包成jar包，适用于Java应用程序和库
2. **war**：Web Application Archive，打包成war包，适用于Java Web应用程序
3. **pom**：Project Object Model，表示该模块是一个父项目，用于管理多个子模块。 声明为pom的模块不会生成可执行的包，而是作为其他模块的父项目，提供统一的配置和依赖管理。

## Plugin 插件

可以把 Maven 想象成一个工厂的流水线，而插件就是流水线上的机器或工人。

* Artifact（工件）: 你生产出来的产品（jar 包）。
* Plugin（插件）: 制造产品的工具。
    ...
    <plugins>
        <plugin>
            ...
            <configuration>
                ...
                <!-- 指定 grpc-java 插件版本，用于生成 gRPC 存根 -->
...
Configuration标签用于配置插件的参数，例如上面示例中配置了protoc编译器和grpc-java插件的版本。

### Executions && Goals
Goal 标签是由插件提供商定义的，表示插件提供的具体功能。

一个插件可能提供了多个目标，而Executions标签用于指定该项目中该插件的需要使用哪些功能（目标）

### Phase

Phase标签用于指定插件的目标在哪个构建阶段执行。如果没有指定phase,那么会根据插件提供商设置的goal对应的默认phase来执行。
