...
运行 Mosquitto 后，本身作为 MQTT Broker运行； 也可以使用 MQTT 客户端工具（如 `mosquitto_pub` 和 `mosquitto_sub`）来发布和订阅消息，从而查看 MQTT Broker 中的消息传递情况。
...
mosquitto_sub -h localhost -p 1883 -t "#" # 订阅所有主题
```
